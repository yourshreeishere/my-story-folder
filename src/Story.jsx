...
export default function Story() {
  ...
  // FIX 2: hudVisible tracks whether HUD should show — true once dive starts, hidden only at y=0
  ...
  // ── FIX 3: Initiate Dive — GSAP scrollTo directly to #section-introduction ──
  const handleDive = useCallback(() => {
    if (diveInitiated) return;
    setDiveInitiated(true);
    setHudVisible(true); // FIX 2: show HUD immediately on dive
    ...
    const tl = gsap.timeline({
      onComplete: () => {
        // Direct smooth scroll to Section 2 with 2-second duration
        gsap.to(window, {
          duration: 2,
          scrollTo: { y: "#section-introduction", offsetY: 0 },
          ease: "power3.inOut",
        });
      },
    });

    // Dive shake animation
  ...
  // ── FIX 4: Restart — full scroll reset + HUD state reset ──────────────────
  const handleRestart = useCallback(() => {
    // Kill any active GSAP scroll tweens
    ...
    // Scroll to top instantly, then GSAP takes over cleanly
    gsap.to(window, {
      duration: 1.2,
      scrollTo: { y: 0 },
      ease: "power3.inOut",
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // FIX 2: HUD visibility — hide only when at very top (scrollY === 0)
      // Once dive is initiated, show HUD unless user is back at the very top
      setHudVisible((prev) => {
        if (sy === 0) return false;
        // Once we've scrolled away from top, keep HUD visible
        return true;
      });
  ...
  return (
    <div ref={rootRef} id="story" className="relative bg-[#001d2d] overflow-hidden">
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          FIX 2: PERSISTENT TELEMETRY HUD
          Shows as soon as user scrolls away from top (scrollY > 0).
          Hidden only when scrollY === 0. Never disappears mid-dive.
      ══════════════════════════════════════════════════════════════════════ */}
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          SECTION 1 — HERO
          FIX 1: Added pt-32 to prevent title clipping under fixed Navbar
      ══════════════════════════════════════════════════════════════════════ */}
      <section id="section-hero" aria-label="Hero"
        ...
        <div className="hero-btn flex flex-col sm:flex-row items-center gap-4">
          ...
          {/* FIX 5: "Learn more" link corrected to match actual section id */}
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          ...
          FIX 3: id updated to "section-introduction" to match scrollTo target
      ══════════════════════════════════════════════════════════════════════ */}
        ...
        <div className="mx-auto max-w-4xl w-full">
          ...
          <div data-story-in className={`rounded-3xl border overflow-hidden mb-12 transition-all duration-700 ${
            ...
            <div className="relative p-10 sm:p-16 text-center">
              ...
              {/* FIX 4: Restart button uses handleRestart for full state + scroll reset */}
              <button
          ...
          {/* ── Archive footer with FIX 5: External links ── */}
          <div data-story-in className="flex flex-col sm:flex-row items-center justify-between gap-6 text-[10px] font-mono text-zinc-700 uppercase tracking-widest border-t border-white/5 pt-8">
            ...
            {/* FIX 5: Work — GitHub link */}
            <div className="flex items-center gap-6">
              <a
                href="https://github.com/yourshreeishere/my-story-folder"
                target="_blank"
              ...
              {/* FIX 5: Contact — mailto link */}
              <a
                href="mailto:bobdevedshree59@gmail.com"
                target="_blank"
                rel="noopener noreferrer"
                className="hover:text-cyan-400 transition-colors flex items-center gap-1"
              >
              ...
              {/* FIX 5: Let's Talk — LinkedIn link */}
              <a
                href="https://www.linkedin.com/in/vedshree-bobde-836150319"
                target="_blank"
                rel="noopener noreferrer"
                className="hover:text-cyan-400 transition-colors flex items-center gap-1"
              >
