...
const PHASES = [
  { id:"surface",    minDepth:0,     maxDepth:199,      label:"PHASE 01 // SURFACE",        transmission:"SYSTEM CHECK: Vessel stabilized. Operator — the water surrounding us didn't start here. 4 billion years ago, Earth was a desert. Every drop of this ocean was hand-delivered by a barrage of ice-comets from deep space. You are diving into an alien's gift." },
  { id:"twilight",   minDepth:200,   maxDepth:999,      label:"PHASE 02 // TWILIGHT ZONE",  transmission:"SONAR SWEEP ACTIVE. We are passing over the Mid-Ocean Ridge — the planet's longest mountain range. It wraps the Earth like a scar, 65,000km long, yet 90% of humanity will never see its peaks. We are officially off the map." },
  { id:"midnight",   minDepth:1000,  maxDepth:3999,     label:"PHASE 03 // MIDNIGHT ZONE",  transmission:"WARNING: Sudden turbulence detected. We've hit the Denmark Strait Cataract. Ahead — an underwater waterfall dropping 3,500 meters into the dark. It moves more water than a thousand Niagaras, and it is completely silent. The ocean is falling into itself." },
  { id:"abyssal",    minDepth:4000,  maxDepth:5999,     label:"PHASE 04 // ABYSSAL PLAIN",  transmission:"VISUAL SENSORS FOCUSED. Scanning a Brine Pool on the seabed. A lake inside an ocean — water so salty it has a surface you could skip a stone across. Those are skeletons at the edges, preserved for decades in a toxic underwater desert. Do not breach the hull." },
  { id:"hadal",      minDepth:6000,  maxDepth:10999,    label:"PHASE 05 // HADAL ZONE",     transmission:"CRITICAL STATUS: Pressure at 15,000 PSI. The weight of 50 jumbo jets presses against your viewport. We are passing through a Mesoscale Eddy — a whirlpool whose mathematics mirror a Black Hole. Information cannot escape. Water cannot mix. We are in a void." },
  { id:"challenger", minDepth:11000, maxDepth:Infinity, label:"PHASE 06 // CHALLENGER DEEP",transmission:"MISSION COMPLETE. We have reached the bottom. We have better maps of the Moon than this spot. Beneath your feet lies Ringwoodite — a blue rock holding three times more water than the surface oceans. We didn't dive into the sea. We found the source." },
];
...
export default function Story() {
  ...
  // ── HUD FIX: use a ref to track scroll position synchronously ──────────────
  // React state batching causes a lag between actual scrollY and the rendered
  // value, which means hudVisible flips back to false a frame too late when
  // scrolling back to top. A ref updates immediately in the event handler so
  // the comparison is always fresh. We still keep scrollY state for snow parallax.
  ...
  const [bottomSettled, setBottomSettled] = useState(false);
  ...
  const handleDive = useCallback(() => {
    ...
    const tl = gsap.timeline({
      onComplete: () => gsap.to(window, { duration: 2, scrollTo: { y: "#section-introduction", offsetY: 0 }, ease: "power3.inOut" }),
    });
  ...
  useEffect(() => {
    ...
    const root = rootRef.current;
    const tl = gsap.timeline({ onComplete: () => setBottomSettled(true) });
    tl.to(root, { x:-16, y:10,  duration:0.05, ease:"none" })
      .to(root, { x:16,  y:-10, duration:0.05, ease:"none" })
      .to(root, { x:-10, y:6,   duration:0.05, ease:"none" })
      .to(root, { x:10,  y:-6,  duration:0.05, ease:"none" })
      .to(root, { x:-5,  y:3,   duration:0.06, ease:"none" })
      .to(root, { x:5,   y:-3,  duration:0.06, ease:"none" })
      .to(root, { x:-2,  y:1,   duration:0.06, ease:"none" })
      .to(root, { x:2,   y:-1,  duration:0.06, ease:"none" })
      .to(root, { x:0,   y:0,   duration:0.6,  ease:"elastic.out(1,0.3)" });
    if (vignetteRef.current) {
      gsap.to(vignetteRef.current, { opacity: 0.9, duration: 0.25 });
      gsap.to(vignetteRef.current, { opacity: 0.3, duration: 1.2, delay: 0.25, ease: "power2.out" });
    }
  }, [isBottom]);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (bgFilterRef.current) {
        const blurAmount    = Math.min(d / 11000 * 6, 6);
        const brightnessVal = Math.max(1 - d / 11000 * 0.55, 0.45);
  ...
  useEffect(() => {
    ...
    const intervals = Array.from(els).map((el) =>
      setInterval(() => {
        ...
        gsap.to(el, { x:(Math.random()-0.5)*intensity, y:(Math.random()-0.5)*intensity, duration:0.08, ease:"none", overwrite:"auto" });
      }, 120)
    );
  ...
  return (
    <div ref={rootRef} id="story" className="relative bg-[#001d2d] overflow-hidden">
      ...
      {/* Vignette */}
      <div ref={vignetteRef} className="fixed inset-0 pointer-events-none z-30 opacity-0"
      ...
      {/* Chroma — only during shake */}
      {isBottom && !bottomSettled && (
        <div className="fixed inset-0 pointer-events-none z-31 chroma-aberration" aria-hidden="true" />
      )}

      {/* Background filter */}
      <div ref={bgFilterRef} className="fixed inset-0 pointer-events-none z-0 transition-none" aria-hidden="true">
      ...
      {/* ── Desktop HUD ── */}
      <div className={`fixed bottom-6 right-6 z-50 font-mono bg-black/90 backdrop-blur-xl rounded-2xl border hidden md:flex flex-col transition-all duration-500 ${
        ...
        <div className="p-5 border-b border-white/5">
          ...
          <p className={`text-[10px] leading-relaxed font-mono min-h-[80px] ${isBottom ? "text-blue-200" : isHadal ? "text-red-200/80" : "text-cyan-100/80"}`}>
            {transmission}<span className="ml-0.5 animate-pulse">▌</span>
          </p>
        </div>
        <div className="p-5">
          ...
          <div className={`text-3xl font-black tracking-tighter tabular-nums pressure-shiver ${
            ...
            : isHadal ? "text-red-400 drop-shadow-[0_0_14px_rgba(239,68,68,0.9)]"
                      : "text-cyan-400"
          } ${isBottom && !bottomSettled ? "chroma-text" : ""}`}>
            {depthDisplay}m
          </div>
      ...
      <section id="section-hero" aria-label="Hero"
        className="relative min-h-screen flex flex-col items-center justify-center text-center px-6 pt-40 pb-24 z-10">
        ...
        <div className="hero-btn flex flex-col sm:flex-row items-center gap-4">
          ...
          <a href="#section-introduction" className="inline-flex items-center gap-2 text-sm text-zinc-500 hover:text-cyan-400 transition-colors font-bold uppercase tracking-widest">
      ...
      <section id="section-introduction" aria-label="Introduction" className="relative z-10 py-32 px-6">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <div data-story-in className="rounded-[2rem] border border-white/5 bg-white/5 p-8 backdrop-blur-2xl ring-1 ring-white/10 flex flex-col justify-center">
              ...
              <a href="https://www.linkedin.com/in/vedshree-bobde-836150319" target="_blank" rel="noopener noreferrer"
                ...
                {CARDS.filter((c) => c.section === "exploration").map((c, index) => {
                  ...
                  return (
                    <div key={c.title} data-story-in
                      ...
                      onMouseMove={(e) => handleCardMouseMove(e, index)} onMouseLeave={() => handleCardMouseLeave(index)}>
      ...
      <section id="section-insight" aria-label="Insight" className="relative z-10 py-24 px-6">
        <div className="mx-auto max-w-6xl">
          ...
          <h2 data-story-in className="text-3xl sm:text-5xl font-black tracking-tighter text-white uppercase mb-4">The <span className="text-red-400">Hadal</span> Zone</h2>
          ...
          <div className="grid gap-10 lg:grid-cols-12 lg:gap-16">
            <div className="lg:col-span-5 lg:sticky lg:top-24 lg:h-fit">
              <div data-story-in className="rounded-[2rem] border border-red-500/20 bg-red-500/5 p-8 backdrop-blur-2xl ring-1 ring-red-500/10">
                ...
                {isHadal && !isBottom && <div className="mt-6 text-[9px] font-black uppercase tracking-widest text-red-400 animate-pulse flex items-center gap-2 pressure-shiver"><span className="size-1.5 rounded-full bg-red-400 inline-block" />Hull Integrity Warning — Extreme Depth</div>}
                {isBottom && <div className="mt-6 text-[9px] font-black uppercase tracking-widest text-blue-300 flex items-center gap-2"><span className="size-1.5 rounded-full bg-blue-400 inline-block animate-ping" />Source Located — Ringwoodite Confirmed</div>}
              </div>
            </div>
            <div className="lg:col-span-7">
                ...
                {CARDS.filter((c) => c.section === "insight").map((c, index) => {
                  ...
                  return (
                    <div key={c.title} data-story-in
                      ...
                      onMouseMove={(e) => handleCardMouseMove(e, rippleIdx)} onMouseLeave={() => handleCardMouseLeave(rippleIdx)}>
                      <SonarRipple x={ripple.x} y={ripple.y} active={ripple.active} />
                      <div className="flex items-center justify-between">
                      ...
                      <div className={`mt-6 text-2xl font-bold text-white transition-colors group-hover:text-red-400 ${isBottom && !bottomSettled ? "chroma-text" : ""}`}>{c.title}</div>
                      <p className="mt-2 text-sm leading-relaxed text-zinc-400">{c.body}</p>
      ...
      <section id="section-conclusion" aria-label="Conclusion"
        ...
        <div className="mx-auto max-w-4xl w-full">
          <div data-story-in className="text-[10px] font-black text-cyan-500 uppercase tracking-[0.4em] mb-6 text-center">Section 05 — Conclusion</div>
          <div data-story-in className={`rounded-3xl border overflow-hidden mb-12 transition-all duration-700 ${
            isBottom ? "border-blue-400/50 shadow-[0_0_80px_rgba(59,130,246,0.35)] bg-gradient-to-b from-blue-950/80 to-black/90" : "border-cyan-500/30 bg-black/80"
          }`}>
            <div className="relative p-10 sm:p-16 text-center">
              <div className={`pointer-events-none absolute inset-0 transition-all duration-1000 ${isBottom ? "bg-[radial-gradient(ellipse_at_50%_50%,rgba(59,130,246,0.25),transparent_60%)]" : "bg-[radial-gradient(circle_at_50%_50%,rgba(34,211,238,0.12),transparent_70%)]"}`} />
              {isBottom && <div className="pointer-events-none absolute inset-0 opacity-10 ringwoodite-crystal" aria-hidden="true" />}
              <h2 className={`relative text-4xl sm:text-6xl font-black tracking-tighter uppercase mb-6 transition-all duration-700 ${isBottom ? "text-blue-200" : "text-white"} ${isBottom && !bottomSettled ? "chroma-text" : ""}`}>
                {isBottom ? "Source" : "Mission"}<br />
                <span className={isBottom ? "text-blue-400" : "text-cyan-400"}>{isBottom ? "Found." : "Complete."}</span>
              </h2>
              <p className={`relative max-w-md mx-auto leading-relaxed mb-4 ${isBottom ? "text-blue-200/80" : "text-zinc-400"}`}>
                {isBottom ? "We have reached the bottom. Beneath your feet lies Ringwoodite — a blue rock holding three times more water than the surface oceans." : "Data collection complete. You have traversed 11,000 metres of ocean, from the sunlit surface to the crushing silence of Challenger Deep."}
              </p>
              {isBottom && <p className="relative text-sm text-blue-300/60 max-w-sm mx-auto leading-relaxed mb-8">We didn't just dive into the sea. We discovered that the Earth is a hollow shell filled with the unknown.</p>}
              <button onClick={handleRestart}
                className={`inline-flex items-center justify-center gap-2 rounded-xl px-10 py-5 text-sm font-bold transition-all hover:scale-105 active:scale-95 ${isBottom ? "bg-blue-500 text-white hover:bg-blue-400 shadow-[0_0_24px_rgba(59,130,246,0.5)]" : "bg-cyan-500 text-black hover:bg-cyan-400"}`}>
                Restart Expedition <ArrowUpRight className="size-4" />
              </button>
            </div>
          </div>

          <div data-story-in className="flex flex-col sm:flex-row items-center justify-between gap-6 text-[10px] font-mono text-zinc-700 uppercase tracking-widest border-t border-white/5 pt-8">
            <div className="flex items-center gap-6">
              <span>Mission Log v2.4</span>
              <span>Deep Sea Archive</span>
            </div>
            <div className="flex items-center gap-6">
              <a href="https://github.com/yourshreeishere/my-story-folder" target="_blank" rel="noopener noreferrer"
                className="hover:text-cyan-400 transition-colors flex items-center gap-1">
                Work <ArrowUpRight className="size-3" />
              </a>
              <a href="mailto:bobdevedshree59@gmail.com"
                className="hover:text-cyan-400 transition-colors flex items-center gap-1">
