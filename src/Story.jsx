/**
 * Story.jsx — Project Re-Genesis: Final Submission Build
 *
 * Architecture:
 *  - Phase 1 (0m)     → Cinematic Story Mode (pinned, typewriter, sonar grid)
 *  - Phase 4 (1000m)  → Cinematic Story Mode (pinned, typewriter, sonar grid)
 *  - Phase 6 (11000m) → Instant VFX: shake + chromatic aberration + blue vignette
 *  - All other phases → HUD typewriter only, active scroll continues
 *
 * Scroll fix: ScrollTo hits Section 1 first, not zone-card-0.
 * Links: mailto + GitHub wired. Restart fully resets scroll + state.
 * Hero: pt-32 prevents navbar overlap. Scroll hint pinned to viewport bottom.
 * All GSAP tweens use force3D: true for GPU compositing.
 */
...
import {
  ArrowUpRight, Sun, Eye, Wind, Zap, Snowflake,
  ChevronDown, Target, ChevronRight,
} from "lucide-react";

gsap.registerPlugin(ScrollTrigger, ScrollToPlugin);

// ─── Constants ────────────────────────────────────────────────────────────────
const EMAIL    = "bobdevedshree59@gmail.com";
const GITHUB   = "https://github.com/yourshreeishere/my-story-folder.git";

// ─── Narrative Phases ─────────────────────────────────────────────────────────
const PHASES = [
  {
    id: "surface", minDepth: 0,     maxDepth: 199,
    label: "PHASE 01 // SURFACE",
    cinematic: true,   // triggers Story Mode pause
  ...
  {
    id: "twilight", minDepth: 200,  maxDepth: 999,
  ...
  {
    id: "midnight", minDepth: 1000, maxDepth: 3999,
  ...
  {
    id: "abyssal", minDepth: 4000,  maxDepth: 5999,
  ...
  {
    id: "hadal", minDepth: 6000,    maxDepth: 10999,
  ...
  {
    id: "challenger", minDepth: 11000, maxDepth: Infinity,
    label: "PHASE 06 // CHALLENGER DEEP",
    cinematic: false,  // VFX slam, not a pause
    transmission: "MISSION COMPLETE. We have reached the bottom. Beneath your feet lies Ringwoodite — a blue rock holding three times more water than the surface oceans. We didn't dive into the sea. We found the source.",
  },
];
...
const CARDS = [
  ...
  { icon: Wind,        title: "1,000m: The Oxygen Minimum", body: "The Bathypelagic Entrance. Oxygen levels drop. Creatures evolved massive gills and slow metabolisms to survive the Dead Zone.",                                          section: "exploration" },
...
// ─── Snow Layers ──────────────────────────────────────────────────────────────
const SNOW_LAYERS = [
  { id:"snow-far",  size:"300px 300px", opacity:0.12, speed:0.08, particles:"radial-gradient(1px 1px at 30px 50px,#fff,transparent),radial-gradient(1px 1px at 80px 120px,#fff,transparent),radial-gradient(1px 1px at 150px 60px,#fff,transparent),radial-gradient(1px 1px at 200px 200px,#fff,transparent),radial-gradient(1px 1px at 250px 90px,#fff,transparent)" },
  { id:"snow-mid",  size:"220px 220px", opacity:0.20, speed:0.18, particles:"radial-gradient(1.5px 1.5px at 25px 40px,#bae6fd,transparent),radial-gradient(1.5px 1.5px at 75px 110px,#bae6fd,transparent),radial-gradient(1.5px 1.5px at 170px 160px,#bae6fd,transparent),radial-gradient(2px 2px at 100px 140px,#bae6fd,transparent)" },
  { id:"snow-near", size:"160px 160px", opacity:0.30, speed:0.32, particles:"radial-gradient(2px 2px at 20px 30px,#e0f2fe,transparent),radial-gradient(2.5px 2.5px at 55px 90px,#e0f2fe,transparent),radial-gradient(3px 3px at 135px 130px,#e0f2fe,transparent),radial-gradient(2.5px 2.5px at 15px 120px,#e0f2fe,transparent)" },
];

// ─── Snow Layer ───────────────────────────────────────────────────────────────
function SnowLayer({ layer, scrollY, speedMult = 1 }) {
  return (
    <div className="absolute inset-0 pointer-events-none" style={{
      ...
      transform: `translate3d(0,${scrollY * layer.speed * speedMult}px,0)`,
    ...
    const schedule = () => {
      ...
      timerRef.current = setTimeout(() => {
        ...
        const iv = setInterval(() => {
          setDisplay(Array.from({ length: String(value).length + 3 }, () =>
...
function SonarRipple({ x, y, active }) {
  ...
  return (
    <div className="pointer-events-none absolute" style={{ left: x, top: y, transform: "translate(-50%,-50%)", zIndex: 20 }}>
      {[0, 1, 2].map((i) => (
        <div key={i} className="absolute rounded-full border border-cyan-400/60" style={{
...
// ─── Sonar Grid (Story Mode background) ──────────────────────────────────────
function SonarGrid() {
  return (
    <svg className="absolute inset-0 w-full h-full pointer-events-none" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* Sonar sweep ring */}
      <circle cx="50%" cy="50%" r="80"  fill="none" stroke="rgba(34,211,238,0.08)" strokeWidth="1" className="sonar-grid-ring" style={{ animationDelay: "0s" }} />
      <circle cx="50%" cy="50%" r="160" fill="none" stroke="rgba(34,211,238,0.05)" strokeWidth="0.8" className="sonar-grid-ring" style={{ animationDelay: "0.6s" }} />
      <circle cx="50%" cy="50%" r="240" fill="none" stroke="rgba(34,211,238,0.03)" strokeWidth="0.5" className="sonar-grid-ring" style={{ animationDelay: "1.2s" }} />
    </svg>
  );
}
...
export default function Story() {
  ...
  const storyModeRef    = useRef(null);   // full-screen cinematic overlay
  const mainContentRef  = useRef(null);   // wrapper around all sections
  ...
  const [storyMode, setStoryMode]       = useState(false);   // cinematic overlay visible
  const [storyPhase, setStoryPhase]     = useState(null);    // which cinematic is showing
  ...
  // HUD typewriter — only active when NOT in story mode
  const hudTransmission = useTypewriter(
    storyMode ? "" : currentPhase.transmission, 26
  );

  // Story mode typewriter — active during cinematic pause
  const storyTransmission = useTypewriter(
    storyPhase ? storyPhase.transmission : "", 22
  );

  // Track which cinematic phases have been shown this session
  const shownCinematics = useRef(new Set());
  const bottomTriggered = useRef(false);

  // ── Story Mode: Enter (fade to navy void) ─────────────────────────────────
  const enterStoryMode = useCallback((phase) => {
    if (storyMode) return;
    shownCinematics.current.add(phase.id);
    setStoryPhase(phase);
    setStoryMode(true);
    ...
    // Fade out main content + HUD
    gsap.to(mainContentRef.current, {
      opacity: 0, duration: 0.6, ease: "power2.inOut", force3D: true,
    });

    // Fade in story overlay
    gsap.fromTo(storyModeRef.current,
      { opacity: 0, display: "flex" },
      { opacity: 1, duration: 0.7, ease: "power2.inOut", force3D: true }
    );
    gsap.set(storyModeRef.current, { display: "flex" });
  }, [storyMode]);

  // ── Story Mode: Exit (fade back to dive) ──────────────────────────────────
  const exitStoryMode = useCallback(() => {
    ...
    // Fade out overlay
    gsap.to(storyModeRef.current, {
      opacity: 0, duration: 0.5, ease: "power2.inOut", force3D: true,
      onComplete: () => gsap.set(storyModeRef.current, { display: "none" }),
    });

    // Fade in main content
    gsap.to(mainContentRef.current, {
      opacity: 1, duration: 0.6, delay: 0.2, ease: "power2.inOut", force3D: true,
    });
  ...
  // ── Keyboard / tap listener for story mode exit ───────────────────────────
  useEffect(() => {
    if (!storyMode) return;
    const onKey = (e) => { if (e.code === "Space" || e.key === " ") { e.preventDefault(); exitStoryMode(); } };
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [storyMode, exitStoryMode]);
  ...
  const handleDive = useCallback(() => {
    ...
    // Phase 1 is cinematic — show story mode first, then scroll on exit
    const phase1 = PHASES[0];
    enterStoryMode(phase1);

    // After story mode exits, scroll to Section Intro (Section 2) — not zone-card-0
    // We hook into the exit to schedule the scroll
    const origExit = exitStoryMode;
    // We override the exit once to attach scroll
    const exitAndScroll = () => {
      document.body.style.overflow = "";
      gsap.to(storyModeRef.current, {
        opacity: 0, duration: 0.5, ease: "power2.inOut", force3D: true,
        onComplete: () => {
          gsap.set(storyModeRef.current, { display: "none" });
          // Scroll to intro section (Section 2)
          gsap.to(window, {
            duration: 2.0,
            scrollTo: { y: "#section-intro", offsetY: 80 },
            ease: "power2.inOut",
            force3D: true,
          });
        },
    ...
    // Temporarily store on ref so keyboard handler uses it
    pendingDiveExit.current = exitAndScroll;
  }, [diveInitiated, enterStoryMode, exitStoryMode]);

  const pendingDiveExit = useRef(null);

  // ── Continue button / Space handler uses pendingDiveExit if set ───────────
  const handleContinue = useCallback(() => {
    if (pendingDiveExit.current) {
      const fn = pendingDiveExit.current;
      pendingDiveExit.current = null;
      fn();
    } else {
      exitStoryMode();
    }
  ...
  // Override space listener to use handleContinue
  useEffect(() => {
    if (!storyMode) return;
    const onKey = (e) => {
      if (e.code === "Space" || e.key === " ") { e.preventDefault(); handleContinue(); }
    };
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [storyMode, handleContinue]);

  // ── Restart Expedition ────────────────────────────────────────────────────
  const handleRestart = useCallback(() => {
    // Reset all state
    setDiveInitiated(false);
    setAtBottom(false);
    setStoryMode(false);
    setStoryPhase(null);
    setDepth(0);
    ...
    shownCinematics.current.clear();
    bottomTriggered.current = false;
    pendingDiveExit.current = null;
    document.body.style.overflow = "";

    // Reset opacity on main content if it was faded
    if (mainContentRef.current) gsap.set(mainContentRef.current, { opacity: 1 });
    if (storyModeRef.current)   gsap.set(storyModeRef.current, { opacity: 0, display: "none" });

    // Clear chromatic / vignette
    if (vignetteRef.current)    gsap.set(vignetteRef.current, { opacity: 0 });

    // Scroll to top
    gsap.to(window, { duration: 1.4, scrollTo: { y: 0 }, ease: "power2.inOut", force3D: true });
  }, []);

  // ── Bottom Impact VFX (Phase 6 — no pause, instant slam) ─────────────────
  useEffect(() => {
    ...
    // Violent single slam shake
    const tl = gsap.timeline();
    tl.to(root, { x:-20, y:14,  duration:0.04, ease:"none", force3D:true })
      .to(root, { x:22,  y:-16, duration:0.04, ease:"none", force3D:true })
      .to(root, { x:-18, y:12,  duration:0.04, ease:"none", force3D:true })
      .to(root, { x:20,  y:-14, duration:0.04, ease:"none", force3D:true })
      .to(root, { x:-14, y:10,  duration:0.05, ease:"none", force3D:true })
      .to(root, { x:14,  y:-10, duration:0.05, ease:"none", force3D:true })
      .to(root, { x:-8,  y:5,   duration:0.06, ease:"none", force3D:true })
      .to(root, { x:8,   y:-5,  duration:0.06, ease:"none", force3D:true })
      .to(root, { x:0,   y:0,   duration:0.5,  ease:"elastic.out(1,0.15)", force3D:true });

    // Blue vignette flicker
    if (vignetteRef.current) {
      gsap.to(vignetteRef.current, { opacity: 1, duration: 0.3, ease: "power2.inOut" });
      gsap.to(vignetteRef.current, {
        opacity: 0.55, duration: 1.8, delay: 0.3,
        ease: "sine.inOut", repeat: 3, yoyo: true,
      });
    }
  }, [isBottom]);
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.to(rootRef.current, {
        backgroundColor: "#000000", force3D: true,
        scrollTrigger: { trigger: rootRef.current, start: "top top", end: "bottom bottom", scrub: true },
      });

      // God ray fade-out
      if (godRayRef.current) {
        gsap.to(godRayRef.current, {
          opacity: 0, force3D: true,
          scrollTrigger: { trigger: rootRef.current, start: "top top", end: "30% top", scrub: true },
        });
      }

      // Section reveals — toggleActions allows reverse on scroll-up
      gsap.utils.toArray("[data-story-in]").forEach((el) => {
        gsap.fromTo(el,
          { y: 40, opacity: 0, filter: "blur(10px)", force3D: true },
          {
            y: 0, opacity: 1, filter: "blur(0px)", duration: 1, ease: "power2.inOut", force3D: true,
            scrollTrigger: {
      ...
      gsap.utils.toArray(".zone-card").forEach((card) => {
        gsap.to(card, {
          opacity: 0, filter: "blur(10px)", y: -40, ease: "none", force3D: true,
      ...
      gsap.utils.toArray(".story-card").forEach((card) => {
        card.addEventListener("mouseenter", () =>
          gsap.to(card, { y: -8, scale: 1.01, duration: 0.4, ease: "power2.inOut", force3D: true }));
        card.addEventListener("mouseleave", () =>
          gsap.to(card, { y: 0, scale: 1, duration: 0.4, ease: "power2.inOut", force3D: true }));
      });

      // Sonar pulse
      gsap.to(".sonar-ping", {
        scale: 4, opacity: 0, duration: 4, repeat: -1, ease: "expo.out", force3D: true,
      });

      // Scanner line
      gsap.to(".scanner-line", {
        y: "100vh", duration: 4.5, repeat: -1, ease: "none", force3D: true,
      });
      ...
      PHANTOMS.forEach((ph) => {
        ...
        if (!el) return;
        gsap.to(el, { x: ph.drift,           duration: ph.duration*0.55, repeat:-1, yoyo:true, ease:"sine.inOut", delay:ph.delay, force3D:true });
        gsap.to(el, { y: ph.drift*0.6,       duration: ph.duration*0.82, repeat:-1, yoyo:true, ease:"sine.inOut", delay:ph.delay+2, force3D:true });
        gsap.to(el, { opacity: ph.opacity*3.5,duration: ph.duration*0.35, repeat:-1, yoyo:true, ease:"sine.inOut", delay:ph.delay+1 });
        gsap.to(el, { scale: 1.08,           duration: ph.duration*0.45, repeat:-1, yoyo:true, ease:"sine.inOut", delay:ph.delay+3, force3D:true });
      });

      // Hero entrance
      gsap.fromTo(".hero-title", { y: 60, opacity: 0, force3D:true }, { y:0, opacity:1, duration:1.6, ease:"power2.inOut", delay:0.3, force3D:true });
      gsap.fromTo(".hero-sub",   { y: 30, opacity: 0, force3D:true }, { y:0, opacity:1, duration:1.2, ease:"power2.inOut", delay:0.7, force3D:true });
      gsap.fromTo(".hero-btn",   { y: 20, opacity: 0, force3D:true }, { y:0, opacity:1, duration:1.0, ease:"power2.inOut", delay:1.1, force3D:true });

      // Stat reveals
      gsap.utils.toArray(".stat-num").forEach((el) => {
        gsap.fromTo(el, { opacity: 0, y: 20, force3D:true },
          { opacity:1, y:0, duration:0.8, ease:"power2.inOut", force3D:true,
    ...
    // ── Scroll tracker ──────────────────────────────────────────────────────
    const handleScroll = () => {
      if (storyMode) return; // don't update depth while pinned
      ...
      setCurrentPhase((prev) => {
        if (prev.id !== phase.id) {
          // Trigger cinematic Story Mode for Phase 3 (midnight) transitions
          // Phase 1 is triggered by dive button, not scroll
          if (phase.cinematic && phase.id !== "surface" && !shownCinematics.current.has(phase.id)) {
            enterStoryMode(phase);
          }
          return phase;
        }
        return prev;
      ...
      // Dynamic CSS filter on bgFilterRef
      if (bgFilterRef.current) {
        const blur  = Math.min(d / 11000 * 6, 6);
        const brit  = Math.max(1 - d / 11000 * 0.55, 0.45);
        const sat   = Math.max(1 - d / 11000 * 0.5, 0.5);
        bgFilterRef.current.style.filter = `blur(${blur}px) brightness(${brit}) saturate(${sat})`;
      }
    };
  ...
  useEffect(() => {
    ...
    const intervals = Array.from(els).map((el) =>
      setInterval(() => {
        ...
        gsap.to(el, { x:(Math.random()-0.5)*intensity, y:(Math.random()-0.5)*intensity, duration:0.08, ease:"none", overwrite:"auto", force3D:true });
      }, 120)
    );
  ...
  return (
    <div ref={rootRef} id="story" className="relative bg-[#001d2d] overflow-hidden">
      ...
      {/* ── Ringwoodite blue vignette ── */}
      <div ref={vignetteRef} className="fixed inset-0 pointer-events-none z-30 opacity-0" aria-hidden="true"
        style={{ background:"radial-gradient(ellipse at center,transparent 30%,rgba(30,80,180,0.55) 70%,rgba(10,30,120,0.85) 100%)", mixBlendMode:"screen" }} />

      {/* ── Chromatic aberration (bottom only) ── */}
      {isBottom && <div className="fixed inset-0 pointer-events-none z-31 chroma-aberration" aria-hidden="true" />}

      {/* ── Background filter wrapper ── */}
      <div ref={bgFilterRef} className="fixed inset-0 pointer-events-none z-0" aria-hidden="true">
        <div ref={godRayRef} className="god-rays absolute inset-0" aria-hidden="true" />
        <div className="absolute inset-0 overflow-hidden">
          {SNOW_LAYERS.map((layer) => (
            <SnowLayer key={layer.id} layer={layer} scrollY={scrollY} speedMult={snowSpeedMult} />
          ))}
        </div>
      </div>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-1" aria-hidden="true">
        {PHANTOMS.map((ph) => (
          <div key={ph.id} id={`phantom-${ph.id}`} style={{
            position:"absolute", top:ph.top, left:ph.left, width:ph.width,
            opacity:ph.opacity, filter:`blur(${ph.blur}px) drop-shadow(0 0 14px ${ph.color})`,
      ...
      {/* ════════════════════════════════════════════════════════════════════
          STORY MODE OVERLAY — Full-screen cinematic pause
          Hidden (display:none) by default. GSAP controls visibility.
      ════════════════════════════════════════════════════════════════════ */}
      <div
        ref={storyModeRef}
        className="story-mode-overlay fixed inset-0 z-[60] hidden flex-col items-center justify-center px-6"
        style={{ background: "#001D2D", opacity: 0 }}
        aria-live="polite"
      >
        ...
        <div className="relative z-10 max-w-2xl w-full text-center">
          ...
          <div className="inline-flex items-center gap-3 mb-8">
            ...
            <span className="text-[9px] font-black uppercase tracking-[0.4em] text-cyan-500/70">
              {storyPhase?.label}
            </span>
            <div className="w-8 h-px bg-cyan-500/40" />
          </div>
          ...
          {/* Typewriter text */}
          <p className="text-base sm:text-lg leading-relaxed font-mono text-cyan-100/90 min-h-[120px] sm:min-h-[100px]">
            {storyTransmission}
            <span className="ml-0.5 animate-pulse text-cyan-400">▌</span>
          </p>

          {/* Corner brackets — decorative military frame */}
          <div className="pointer-events-none absolute -inset-6 sm:-inset-10">
            <div className="absolute top-0 left-0 w-6 h-6 border-t border-l border-cyan-500/30" />
            ...
            <div className="absolute bottom-0 right-0 w-6 h-6 border-b border-r border-cyan-500/30" />
          </div>
        </div>
        ...
        <div className="relative z-10 mt-16 flex flex-col items-center gap-4">
          {/* Desktop: Space hint */}
          <p className="hidden sm:block text-[10px] uppercase tracking-[0.4em] text-cyan-500/40 font-bold animate-pulse">
            Press Space to Continue
          </p>
          {/* Mobile: Visible button */}
          <button
            onClick={handleContinue}
            className="sm:hidden inline-flex items-center gap-2 rounded-xl border border-cyan-500/30 bg-cyan-500/10 px-6 py-3 text-sm font-black uppercase tracking-widest text-cyan-300 active:scale-95 transition-transform"
          >
            Continue Dive <ChevronRight className="size-4" />
          </button>
          {/* Desktop also gets a subtle clickable hint */}
          <button
            onClick={handleContinue}
            className="hidden sm:inline-flex items-center gap-2 text-[10px] text-cyan-500/25 hover:text-cyan-500/60 transition-colors uppercase tracking-widest font-bold"
          >
            or click here
          </button>
        </div>
      ...
      <div className={`fixed bottom-6 right-6 z-50 font-mono bg-black/90 backdrop-blur-xl rounded-2xl border hidden md:flex flex-col transition-all duration-700 ${
        storyMode ? "opacity-0 pointer-events-none" : "opacity-100"
      } ${
        ...
        <div className="p-5 border-b border-white/5">
          ...
          <p className={`text-[10px] leading-relaxed font-mono min-h-[72px] ${isBottom ? "text-blue-200" : isHadal ? "text-red-200/80" : "text-cyan-100/80"}`}>
            {hudTransmission}
        ...
        {/* Telemetry */}
        <div className="p-5">
          <div className={`text-[9px] uppercase tracking-[0.3em] mb-2 ${isBottom ? "text-blue-400/70" : isHadal ? "text-red-500/70" : "text-cyan-500/60"}`}>Live Telemetry</div>
          <div className={`text-3xl font-black tracking-tighter tabular-nums pressure-shiver ${isBottom ? "text-blue-300 drop-shadow-[0_0_20px_rgba(147,197,253,0.9)] chroma-text" : isHadal ? "text-red-400 drop-shadow-[0_0_14px_rgba(239,68,68,0.9)]" : "text-cyan-400"}`}>
            {depthDisplay}m
          </div>
          ...
          <div className="w-full h-1.5 rounded-full bg-white/10 mb-3 overflow-hidden">
            <div className={`h-full rounded-full transition-all duration-300 ${isBottom ? "bg-blue-400 shadow-[0_0_14px_rgba(147,197,253,0.9)]" : isHadal ? "bg-red-400 shadow-[0_0_12px_rgba(239,68,68,0.9)]" : "bg-cyan-400"}`}
      ...
      <div className={`fixed bottom-4 left-4 right-4 z-50 md:hidden font-mono bg-black/90 backdrop-blur-xl px-4 py-3 rounded-xl border transition-all duration-500 ${
        ...
        <div className={`text-[8px] mb-2 font-mono leading-relaxed ${isBottom ? "text-blue-300" : isHadal ? "text-red-300" : "text-cyan-300"}`}>
          ...
          {hudTransmission.slice(0, 80)}{hudTransmission.length > 80 ? "…" : ""}
        </div>
      ...
      {/* ════════════════════════════════════════════════════════════════════
          MAIN CONTENT WRAPPER (fades during Story Mode)
      ════════════════════════════════════════════════════════════════════ */}
      <div ref={mainContentRef}>

        {/* ── SECTION 1: HERO ──────────────────────────────────────────── */}
        <section id="section-hero" aria-label="Hero"
          className="relative min-h-screen flex flex-col items-center justify-center text-center px-6 pt-32 pb-24 z-10">
          ...
          <div className="hero-btn flex flex-col sm:flex-row items-center gap-4">
            ...
            <a href={GITHUB} target="_blank" rel="noreferrer"
              ...
              View Work <ChevronRight className="size-4" />
            </a>
          </div>

          {/* Scroll hint — pinned to viewport bottom via fixed positioning within section */}
          <div className="fixed bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 text-zinc-600 pointer-events-none z-10"
            style={{ display: diveInitiated ? "none" : "flex" }}>
        ...
        <section id="section-intro" aria-label="Introduction" className="relative z-10 py-32 px-6">
          <div className="mx-auto max-w-6xl">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16">
              {[{ num:"95%",label:"Ocean unexplored"},{ num:"11km",label:"Max depth recorded"},{ num:"1,100",label:"Atmospheres of pressure"},{ num:"~2°C",label:"Hadal temperature"}].map((s) => (
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div data-story-in className="rounded-[2rem] border border-white/5 bg-white/5 p-8 backdrop-blur-2xl ring-1 ring-white/10">
                ...
                <ul className="space-y-5">
                  {[{t:"Structural Adaptation",c:"bg-cyan-500"},{t:"Atmospheric Density",c:"bg-blue-600"},{t:"Bioluminescent Signals",c:"bg-indigo-600"},{t:"Hadal Pressure Systems",c:"bg-violet-600"}].map((item) => (
              ...
              <div data-story-in className="rounded-[2rem] border border-white/5 bg-white/5 p-8 backdrop-blur-2xl ring-1 ring-white/10 flex flex-col justify-center">
                <p className="text-lg text-zinc-400 leading-relaxed">As the light fades, the biology changes. You are entering a world where bone dissolves and bioluminescence is the only currency.</p>
                <p className="mt-4 text-sm text-zinc-600 leading-relaxed">Below 200m photosynthesis ceases. Below 1,000m permanent darkness reigns. Every creature here has evolved solutions no surface organism has ever faced.</p>
                {/* Let's Talk → mailto */}
                <a href={`mailto:${EMAIL}`}
                  className="mt-8 inline-flex items-center gap-2 text-sm font-bold text-cyan-400 hover:text-cyan-300 transition-colors uppercase tracking-widest">
            ...
            <div className="grid gap-10 lg:grid-cols-12 lg:gap-16">
              <div className="lg:col-span-5 lg:sticky lg:top-24 lg:h-fit">
                <div data-story-in className="rounded-[2rem] border border-white/5 bg-white/5 p-8 backdrop-blur-2xl ring-1 ring-white/10">
                  ...
                  <p className="mt-8 text-sm text-zinc-400 leading-relaxed">The upper ocean is deceptively familiar. Yet below 200m the world transforms irreversibly — pressure climbs, light vanishes, and evolution produces solutions you won't find on the surface.</p>
                </div>
              </div>
                ...
                <div data-story-in className="rounded-[2rem] border border-red-500/20 bg-red-500/5 p-8 backdrop-blur-2xl ring-1 ring-red-500/10">
                  ...
                  {isHadal && !isBottom && (
                    <div className="mt-6 text-[9px] font-black uppercase tracking-widest text-red-400 animate-pulse flex items-center gap-2 pressure-shiver">
                      <span className="size-1.5 rounded-full bg-red-400 inline-block" />Hull Integrity Warning
                    </div>
                  )}
                  {isBottom && (
                    <div className="mt-6 text-[9px] font-black uppercase tracking-widest text-blue-300 flex items-center gap-2">
                      <span className="size-1.5 rounded-full bg-blue-400 inline-block animate-ping" />Source Located — Ringwoodite
                    </div>
                  )}
                </div>
              </div>
        ...
        {/* ── SECTION 5: CONCLUSION — Deep Sea Archive ─────────────────── */}
        <section id="section-conclusion" aria-label="Conclusion"
          ...
          <div className="mx-auto max-w-4xl w-full">
            <div data-story-in className="text-[10px] font-black text-cyan-500 uppercase tracking-[0.4em] mb-6 text-center">
              Section 05 — Deep Sea Archive
            </div>
            <div data-story-in className={`rounded-3xl border overflow-hidden mb-12 transition-all duration-700 ${
              isBottom ? "border-blue-400/50 shadow-[0_0_80px_rgba(59,130,246,0.35)] bg-gradient-to-b from-blue-950/80 to-black/90"
                       : "border-cyan-500/30 bg-black/80"
            }`}>
              <div className="relative p-10 sm:p-16 text-center">
                <div className={`pointer-events-none absolute inset-0 transition-all duration-1000 ${
                  isBottom ? "bg-[radial-gradient(ellipse_at_50%_50%,rgba(59,130,246,0.25),transparent_60%)]"
                           : "bg-[radial-gradient(circle_at_50%_50%,rgba(34,211,238,0.12),transparent_70%)]"
                }`} />
                {isBottom && <div className="pointer-events-none absolute inset-0 opacity-10 ringwoodite-crystal" aria-hidden="true" />}
                <h2 className={`relative text-4xl sm:text-6xl font-black tracking-tighter uppercase mb-6 transition-all duration-700 ${isBottom ? "text-blue-200 chroma-text" : "text-white"}`}>
                  {isBottom ? "Source" : "Mission"}<br />
                  <span className={isBottom ? "text-blue-400" : "text-cyan-400"}>
                    {isBottom ? "Found." : "Complete."}
                  </span>
                </h2>
                <p className={`relative max-w-md mx-auto leading-relaxed mb-4 ${isBottom ? "text-blue-200/80" : "text-zinc-400"}`}>
                  {isBottom
                    ? "Beneath your feet lies Ringwoodite — a blue rock holding three times more water than the surface oceans."
                    : "Data collection complete. You have traversed 11,000 metres of ocean, from sunlit surface to crushing silence."}
                </p>
                {isBottom && (
                ...
                {/* Restart — fully resets scroll + state */}
                <button onClick={handleRestart}
                  className={`inline-flex items-center justify-center gap-2 rounded-xl px-10 py-5 text-sm font-bold transition-all hover:scale-105 active:scale-95 ${
                    isBottom ? "bg-blue-500 text-white hover:bg-blue-400 shadow-[0_0_24px_rgba(59,130,246,0.5)]"
            ...
            <div data-story-in className="flex flex-col sm:flex-row items-center justify-between gap-4 text-[10px] font-mono text-zinc-700 uppercase tracking-widest border-t border-white/5 pt-8">
              ...
              <div className="flex items-center gap-6">
                <a href={`mailto:${EMAIL}`} className="hover:text-zinc-400 transition-colors">Contact</a>
                <a href={GITHUB} target="_blank" rel="noreferrer" className="hover:text-zinc-400 transition-colors">GitHub</a>
